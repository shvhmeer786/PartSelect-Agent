#!/usr/bin/env python3
"""
Test script for the enhanced CatalogClient and DocsClient classes.
"""
...
from modules.tools import CatalogClient, DocsClient

async def test_catalog_client():
    """Test the functionality of the CatalogClient."""
...
async def test_docs_client():
    """Test the functionality of the DocsClient."""
    client = DocsClient()
    ...
    installation_docs = await client.get_installation_docs("heating element", "dishwasher")
    print("\nInstallation docs for 'heating element' (dishwasher):")
    for doc in installation_docs:
        print(f"  {doc['title']}")
    
    # Test getting troubleshooting docs
    troubleshooting_docs = await client.get_troubleshooting_docs("ice maker", "refrigerator")
