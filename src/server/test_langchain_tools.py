...
async def test_product_lookup_tool():
    ...
    result = await lookup_tool._run_async(invalid_part)
    print(f"Result: {result}")

# Test the CompatibilityTool
async def test_compatibility_tool():
    ...
    # Create the tool
    compat_tool = CompatibilityTool(catalog_client=catalog_client)
    ...
    result = await compat_tool._run_async(query)
    print(f"Result: {result}")
...
# Test the InstallationGuideTool
async def test_installation_guide_tool():
    print("\n===== Testing InstallationGuideTool =====")
    
    # Create async clients with mock data
    docs_client = AsyncDocsClient(mongodb_uri="mongodb://localhost:27017", use_mock=True)
    
    # Create the tool
    install_tool = InstallationGuideTool(docs_client=docs_client)
    ...
    result = await install_tool._run_async(query)
...
# Test the ErrorDiagnosisTool
async def test_error_diagnosis_tool():
    print("\n===== Testing ErrorDiagnosisTool =====")
    ...
    # Create the tool
    diagnosis_tool = ErrorDiagnosisTool(docs_client=docs_client, catalog_client=catalog_client)
    
    # Test with a valid problem and appliance type
    query = "not making ice:refrigerator"
    print(f"Diagnosing problem: {query}")
    result = await diagnosis_tool._run_async(query)
    print(f"Result preview (first 200 chars):\n{result[:200]}...\n")
